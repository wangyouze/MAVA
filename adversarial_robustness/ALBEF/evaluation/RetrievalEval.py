...
sys.path.append('/home/user/wangyouze/projects/adversarial_robustness/')
...
sys.path.append('/home/user/wangyouze/projects/adversarial_robustness/train')
import utils
from torchvision import transforms

sys.path.append('/home/user/wangyouze/projects/adversarial_robustness/dataset')
from caption_dataset import pair_dataset
...
def retrieval_eval(model, model_ad, ref_model, data_loader, tokenizer, device, config, args):
    # test
    model.eval()
    ref_model.eval()
    ...
    multi_attacker = MultiModalAttacker(model, image_attacker, text_attacker, tokenizer, cls=True)
    ...
    for images, texts, texts_ids in data_loader:
        ...
        with torch.no_grad():
            ...
            output = model_ad.inference(images, texts_input, use_embeds=False)
...
def retrieval_score(model, image_feats, image_embeds, text_feats, text_embeds, text_atts, num_image, num_text, device=None):
    ...
    for i, sims in enumerate(metric_logger.log_every(sims_matrix, 5000, header)):
...
def main(args, config):
    ...
    # visual_encoder_state_dict = torch.load("/home/user/wangyouze/projects/save/image-text-retrieval/ALBEF/TVT_ALBEF_lr_0.001_eps_4.0_stepsize_1.0_N_3_pgd_2/ViT-B_16_openai_coco_l2_coco_iSy3m/checkpoints/step_10.pt", map_location='cuda')
    visual_encoder_state_dict = torch.load("/home/user/wangyouze/projects/save/image-text-retrieval/ALBEF/TVT_ALBEF_lr_0.001_eps_2.0_stepsize_1.0_N_3_pgd_2/ViT-B_16_openai_coco_l2_coco_hllIk/checkpoints/step_10.pt", map_location='cuda')
    ...
    score_i2t, score_t2i = retrieval_eval(model,model_ad, ref_model, test_loader, tokenizer, device, config, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='/home/user/wangyouze/projects/adversarial_robustness/configs/Retrieval_flickr.yaml')
    parser.add_argument('--output_dir', default='output/retrieval')
    parser.add_argument('--checkpoint', default="/home/user/wangyouze/projects/checkpoints/ALBEF/flickr30k.pth")
    # parser.add_argument('--checkpoint', default="/home/user/wangyouze/projects/adversarial_robustness/ALBEF/checkpoints/mscoco.pth")
    parser.add_argument('--text_encoder', default='/home/user/wangyouze/projects/checkpoints/bert-base-uncased/', type=str)
    ...
    parser.add_argument('--eps', default=4.0, type=float)
