...
class CLIP(nn.Module):
    ...
    def inference_image(self, image):
        ...
        return {'image_embed': F.normalize(self.encode_image(image), dim=-1)}
    ...
    def inference_text(self, text_input):
        ...
        for input_ids in text_input.input_ids:
            ...
            text.append(t)
        text_input = clip.clip.tokenize(text, 77, True).to(self.logit_scale.device)

        return {'text_embed': F.normalize(self.encode_text(text_input), dim=-1)}
        '''
        x = self.token_embedding(text_input).type(self.dtype)  # [batch_size, n_ctx, d_model]
