...
sys.path.append('/data/home/wangyouze/projects/Adversarial-Training-against-Multimodal-Adversarial-Attacks/adversarial_robustness/ALBEF')
from imageAttacker import pgd_2
...
parser.add_argument('--stepsize_adv', type=float, default=0.5, help='Step size for adversarial attack (no effect for apgd)')
...
parser.add_argument('--checkpoint', default="/data/home/wangyouze/MultimodalAttack/Co-attack/checkpoints/TCL/checkpoint_flickr_finetune.pth")
...
def main(args, config):
	...
	model_orig = load_model(tokenizer, args, device=local_rank)
	model_orig = DDP(model_orig, device_ids=[local_rank], output_device=local_rank)
	
	model = load_model(tokenizer, args, device=local_rank)
...
def train_one_epoch(
	...
	for batch_idx, (images, texts_group, images_ids, text_ids_groups) in enumerate(dataloader):
		...
		for i in range(len(texts_group)):
			if len(texts_group[i]) >= 5:
				texts += texts_group[i][:5]
				texts_ids += text_ids_groups[i][:5]
				txt2img += [i]*len(text_ids_groups[i][:5])
			else:
				texts += texts_group[i]
		...
		images = images.to(device).repeat_interleave(5, dim=0)       
	...
	# return sum(loss_buffer)/len(loss_buffer), sum(loss_1_buffer)/len(loss_1_buffer),  sum(loss_2_buffer)/len(loss_2_buffer),  sum(loss_3_buffer)/len(loss_3_buffer)
...
if __name__ == '__main__':
	...
	args.output_dir = f'/data/home/wangyouze/projects/save/adversarial_robustness/TCL/TVT_TCL_lr_{args.lr}_eps_{args.eps}_stepsize_{args.stepsize_adv}_N_{args.num_neighbor_sampling}_1128'
	args.save_dir = f'/data/home/wangyouze/projects/save/adversarial_robustness/TCL/TVT_TCL_lr_{args.lr}_eps_{args.eps}_stepsize_{args.stepsize_adv}_N_{args.num_neighbor_sampling}_1128'
